...
class BrowserWindow(tk.Toplevel, ABC):
    def __init__(self, parent):
        ...
        self.scrollable_table = ScrollableTableFrame(self)

        self.table = self.scrollable_table.table
        self.table.change_table_style(table_style)
        action_bar = self.make_action_bar()

        self.table.configure(bg='#cacaca')

        self.scrollable_table.place(x=0, y=0, relheight=1, height=-ACTION_BAR_HEIGHT, relwidth=1)
